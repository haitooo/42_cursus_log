...
static void	createForm(Intern const & intern, AForm** forms, int & count)
{
	if (count >= MAX_FORM)
	{
	...
	std::string	form_name = prompt("Enter Form Name. usable: " + Intern::usableFormNames());
	std::string	target = prompt("Enter Target");

	forms[count] = intern.makeForm(form_name, target);
	count++;
